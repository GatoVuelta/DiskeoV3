...
if (http_status == 200) && (fetched_data != "") && (status == 0)
{
	...
	global.assets_list = ds_map_find_value(json_data, "default");
	var l_size = ds_list_size(global.assets_list);
	show_debug_message(string("list size is: ")+string(l_size));
	
	//Add to dropwdown list
	for (var n = 0; n < l_size; n++;)
	{
		asset_map = ds_list_find_value(global.assets_list, n);
		
		//ds_map_add(asset_map, string("r")+string(n), false);
